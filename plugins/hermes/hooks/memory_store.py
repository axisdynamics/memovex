#!/usr/bin/env python3
"""
memovex — Hermes Stop Hook.

Strategy (API-first with direct fallback):
  1. Try POST localhost:7914/api/hermes/store (x2: user + assistant)
  2. If API unavailable → fall back to direct HermesMemoryPlugin.sync_turn()
"""
...
def main() -> None:
    ...
    ok = _store_via_api(f"User: {user_msg[:500]}", ["user_turn"],
                        session_id, 0.7, 0.5)
    ok = _store_via_api(f"Hermes: {asst_msg[:800]}", ["assistant_turn"],
    ...
    if not ok:
        try:
            ...
            try:
                plugin.sync_turn(user_msg[:500], asst_msg[:800],
                                 session_id=session_id)
            finally:
                try:
                    plugin.shutdown()
